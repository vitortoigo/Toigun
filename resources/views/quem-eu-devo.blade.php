@extends('layout.main')
@section('title', 'Toigun | Quem eu devo')
...
<div class="px-4 md:px-10 py-4 md:py-7 bg-gray-700 rounded-tl-lg rounded-tr-lg">
    <div class="sm:flex items-center justify-between">
        <p tabindex="0" class="focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-white">Novo cadastro (quem eu devo)</p>
    </div>
    <form method="post" action="{{ route('quem-eu-devo.novo') }}">
        @csrf
        <div class="flex flex-wrap -mx-2 mt-10 items-end">
            <div class="w-full px-2 md:w-1/3">
                <label class="block mb-1 text-white" for="formGridCode_card">Nome de quem eu devo</label>
                <input name="qed[nome]" class="w-full h-10 px-3 text-base placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text">
            </div>
            <div class="w-full px-2 md:w-1/6">
                <label class="block mb-1 text-white" for="formGridCode_name">Valor à pagar</label>
...
@if($quem_eu_devo->count() > 0)
<div class="px-4 md:px-10 py-4 md:py-7 bg-gray-700 rounded-tl-lg rounded-tr-lg mt-10">
    <div class="sm:flex items-center justify-between">
        <p tabindex="0" class="focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-white">Lista à quem eu devo</p>
    </div>
</div>
<div class="bg-gray-800  shadow px-4 md:px-10 py-10 md:pt-7 overflow-y-auto">
        ...
        <tbody class="w-full">
            ...
            <tr tabindex="0" class="focus:outline-none h-20 text-sm leading-none text-white bg-gray-800 hover:bg-gray-900  border-b border-t border-gray-700 ">
                <td class="pl-4 cursor-pointer">
                    <p class="text-sm font-medium leading-none text-white ">{{ $qed->nome }}</p>
                </td>
                <td class="pl-12">
                    <p class="text-sm font-medium leading-none text-white ">R${{ money_format($qed->valor) }}</p>
                </td>
                <td class="">
                    <form method="post" action="{{ route('quem-eu-devo.concluido', $qed->id) }}">
                        @csrf
                        @method('delete')
                        <input id="checkConcluido" class="hidden" type="checkbox" name="concluido">
                        <button id="concluido" type="submit" class="w-10 focus:ring-2 rounded-md focus:outline-none" role="button" aria-label="options">
    ...
    <div class="mt-5">
        <p class="focus:outline-none text-base sm:text-lg md:text-xl font-bold leading-normal text-white">Total: <span style="color: red">R${{ money_format($qedSoma) }}</span></p>
    </div>
    <div class="mt-5">
        {{ $quem_eu_devo->links() }}
    </div>
